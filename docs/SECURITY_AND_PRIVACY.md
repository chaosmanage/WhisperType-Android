...
WhisperType Android is an on-device overlay and dictation client with a direct-to-API architecture. It operates with zero intermediary servers, no accounts, and no telemetry. Its security model is centered on keeping the Gemini API key encrypted on the device, keeping transcripts and audio out of storage and logs by default, and keeping the Accessibility Service's reach minimal. This document is the security and privacy reference for the app and describes how it handles secrets, transcripts, history, the clipboard, logging, Accessibility access, the microphone, and third-party API communication.
...
- **No backend server.** Audio streams directly from the device to the Gemini Live API over TLS. There is no WhisperType cloud account, database, or backend. Data governance at the API endpoint is governed by Google's Gemini API terms: free-tier keys are subject to Google model training and human review, whereas paid-tier keys (with an active billing account) are not used to train Google models.
