 ...
 * Live model ([GeminiSessionFactory.LIVE_MODEL]) over `BidiGenerateContent`.
 * Only that model is free with the owner's API key; every other Gemini model
 * would incur charges and is explicitly refused. Audio goes only to Gemini
 * Live; text shaping runs only on Groq's free tier.
 *
...
class ModelPolicyTest {
    ...
    fun `no non-Live Gemini endpoint appears in production sources`() {
        ...
        // OTHER Gemini models, and the Files API is their upload path. None of
        // them may exist in this app.
    ...
    fun `audio never leaves to a non-Gemini-Live service`() {
        // Audio goes only to Gemini Live. Groq is text-only: its speech
        // endpoints (Whisper transcription/translation) are forbidden.
