...
- **Full hero word count (`ui`)** — the Words-last-7-days headline now shows the complete grouped number (e.g. `1,024`) instead of abbreviating to `1.0k` past a thousand.

## [1.2.6] - 2026-09-14

### Fixed

- **At-a-glance card labels (`ui`)** — shortened the metric labels to **Words / min** and **Words / session** so they fit the two-column cards instead of truncating, allowed labels to wrap to two lines under large font sizes, and equalized side-by-side card heights so a wrapped card no longer leaves its neighbor short.
