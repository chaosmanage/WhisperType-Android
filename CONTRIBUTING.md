...
Guidelines for working on this repository: branch workflow, scope, tests, documentation, commit style, and the privacy rules that apply to every change (Implementation Plan sections 21 and 14.4).

## Repository layout

- `app/src/main/java/com/whispertype/android/` — grouped by feature package: `accessibility`, `audio`, `dictation`, `gemini`, `overlay`, `validation`, `security`, `settings`, `onboarding`, `history`, `diagnostics`.
...
Local Git with a stable `main` and short-lived feature branches, per Implementation Plan section 21:
...
- `SourcePrivacyAuditTest` (in `app/src/test`) scans the source tree as part of unit tests and fails on API key literals, authenticated URL literals, sensitive log lines, and committed keystore/secret files. Treat it as a hard gate.
